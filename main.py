...
def quiz(lesson_id):
    ...
    # GET запрос - без предварительно выбранных ответов
    return render_template("quiz.html",
                           quiz=quiz_obj,
                           questions=questions,
                           lesson_id=lesson_id,
...
def check_achievements(user_id):
    ...
    if progress.completed_lessons:
        for lesson_id in progress.completed_lessons.split(','):
            lesson = db_sess.get(Lesson, int(lesson_id))
            if lesson:
                completed_modules.add(lesson.module_id)
...
def leaderboard():
    db_sess = get_db()
    data = db_sess.query(User.name, UserProgress.completed_lessons, UserProgress.total_xp).join(
        User, User.id == UserProgress.user_id).order_by(UserProgress.total_xp.desc()).all()
